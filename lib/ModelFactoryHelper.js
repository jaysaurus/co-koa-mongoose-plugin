...
const ModelFactoryHelper = stampit({
  init () {
    function bindHooks (schema, hooks) {
      if (typeof hooks === 'object') {
        Object.keys(hooks).forEach(key => {
          ['pre', 'post'].forEach(hook => {
            if (hooks[hook] && typeof hooks[hook] === 'object') {
              ['count', 'find', 'findOne', 'findOneAndRemove', 'findOneAndUpdate', 'update', 'save', 'validate', 'update'].forEach(
                func => {
                  if (hooks[hook].hasOwnProperty(func) && typeof hooks[hook][func] === 'function') {
